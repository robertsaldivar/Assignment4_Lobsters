...
```{r}
# 8 Running test to see if there is a difference between site in 2012 and 2017
lobster_MPA_2012_17 <- lobster_size_edits %>%
  filter(YEAR=="2012" |YEAR=="2017", SITE=="NAPL"| SITE=="IVEE") %>%
  as.data.frame() %>%
  expand.dft(freq="COUNT")

lobster_nonMPA_2012_17 <- lobster_size_edits %>%
  filter(YEAR=="2012" |YEAR=="2017", SITE=="AQUE"| SITE=="MOHK" |SITE=="CARP") %>%
  as.data.frame() %>%
  expand.dft(freq="COUNT")
